  ...
  return (
    <div style={{ background:'rgba(0,20,0,0.7)', border:'1px solid #00ff6644', borderLeft:'3px solid #00ff88', borderRadius:'14px', padding:'1.2rem', marginBottom:'12px' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'8px' }}>
        <span style={{ fontSize:'15px', fontWeight:600, color:'#e0ffe0' }}>{task.titre}</span>
        <span style={{ fontSize:'11px', padding:'3px 10px', borderRadius:'20px', fontWeight:600, ...badgeStyle[task.statut] }}>{task.statut}</span>
      </div>
      ...
      <div style={{ display:'flex', gap:'8px' }}>
        ...
        <button onClick={() => onDelete(task.id)} style={{ background:'rgba(255,60,60,0.1)', border:'1px solid #ff444444', borderRadius:'6px', padding:'5px 14px', color:'#ff6b6b', fontSize:'12px', cursor:'pointer' }}>✕ Supprimer</button>
        <Link to={`/task/${task.id}`} style={{ marginLeft:'auto', background:'rgba(0,255,100,0.1)', border:'1px solid #00ff6644', borderRadius:'6px', padding:'5px 14px', color:'#00ff88', fontSize:'12px', textDecoration:'none' }}>→ Détail</Link>
      </div>
    </div>
  )
}
